...
export function SuperAdminPatientTrends() {
  ...
  const { dailyCount, monthlyCount, yearlyCount, chartData, totalInRange } = useMemo(() => {
    const now = new Date()
    const sourceDates = patients
      .map((patient) => new Date(patient.createdAt || patient.created_at || ""))
    ...
    <Card className="border-none glass-premium rounded-3xl animate-in fade-in slide-in-from-bottom-6 duration-1000 delay-150 overflow-hidden group h-full flex flex-col">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between gap-3">
          <div>
            <CardTitle className="text-xl font-black text-slate-900 dark:text-white">Patient Activity Analytics</CardTitle>
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">Daily, monthly, and yearly patient additions in current hospital</p>
          </div>
      ...
      <CardContent className="flex-1 flex flex-col gap-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/10 p-3 bg-white/40 dark:bg-white/5">
            <p className="text-[10px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400">Today</p>
            <p className="text-2xl font-black text-slate-900 dark:text-white">{dailyCount}</p>
          </div>
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/10 p-3 bg-white/40 dark:bg-white/5">
            <p className="text-[10px] font-black uppercase tracking-widest text-emerald-600 dark:text-emerald-400">This Month</p>
            <p className="text-2xl font-black text-slate-900 dark:text-white">{monthlyCount}</p>
          </div>
          <div className="rounded-2xl border border-slate-200/60 dark:border-white/10 p-3 bg-white/40 dark:bg-white/5">
            <p className="text-[10px] font-black uppercase tracking-widest text-amber-600 dark:text-amber-400">This Year</p>
