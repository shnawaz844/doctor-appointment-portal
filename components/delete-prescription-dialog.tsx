...
export function DeletePrescriptionDialog({ prescriptionId, medicationName, children }: DeletePrescriptionDialogProps) {
      ...
      <DialogContent className="max-w-md bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-slate-200 dark:border-slate-800 shadow-2xl rounded-3xl p-0 overflow-hidden">
        <DialogHeader className="px-8 pt-8 pb-4 border-b border-slate-100 dark:border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <DialogTitle className="text-xl font-black text-slate-900 dark:text-white">Delete Prescription</DialogTitle>
          </div>
        </DialogHeader>
        <div className="p-8 space-y-4">
          <DialogDescription className="text-slate-600 dark:text-slate-400 font-medium leading-relaxed">
            Are you sure you want to delete the prescription for <span className="font-bold text-slate-900 dark:text-white">{medicationName}</span>? 
